...
acs_vars_commute <- c(
  ...
  "B08012_006", "B08012_007", "B08012_008", "B08012_009",  # Commute <40 min (추가)
  "B08012_010", "B08012_011",  # Commute 40~60 min
  "B08012_012", "B08012_013"   # Commute 60+ min
)


#  **2 Rent Burden-related Variables (Tract Level)**
acs_vars_rent <- c(
...
santa_clara_commute <- get_acs(
  geography = "tract",  #  Block Group → Tract  
...
# Additional Control Variables (Block Group Level → Needs Aggregation to Tract)
acs_vars_control <- c(
  "B19013_001",  # Median Household Income(Block Group Level)
  "B01003_001",  # Total Population(Block Group Level)
  "B25010_001"   # Total Housing Units(Block Group Level)
)

# Retrieve Control Variables from ACS (Block Group Level)
santa_clara_control <- get_acs(
  geography = "block group",   
  variables = acs_vars_control,
  state = state_fips,
  county = county_fips,
  year = 2023
) %>%
  mutate(GEOID = substr(GEOID, 1, 11))  # Convert Block Group ID to Tract ID (Keep first 11 digits)

# Aggregate Block Group Data to Tract Level
santa_clara_control <- santa_clara_control %>%
  pivot_wider(names_from = variable, values_from = estimate, values_fill = list(estimate = NA)) %>%
  group_by(GEOID) %>%
  summarize(
    median_income = mean(B19013_001, na.rm = TRUE),  # Median Household Income (Mean)
    total_population = sum(B01003_001, na.rm = TRUE),  # Total Population (Sum)
    total_housing_units = sum(B25010_001, na.rm = TRUE)  # Total Housing Units (Sum)
  ) %>%
  mutate(
    pop_density = total_population / total_housing_units  # Population Density Calculation
  ) %>%
  select(GEOID, median_income, pop_density)

# Merge Aggregated Control Variables with Main Dataset
santa_clara_final <- left_join(santa_clara_final, santa_clara_control, by = "GEOID")
...
# Run OLS Regression Analysis (Including Control Variables)
ols_model <- lm(
  commute_40plus_pct ~ rent_burden_50_plus_pct + rent_burden_below_50_pct +
    median_income + pop_density,  
